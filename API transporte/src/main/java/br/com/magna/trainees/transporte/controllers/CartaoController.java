...
public class CartaoController extends GenericController {
    ...
    @ApiResponse(responseCode = "200", description = "Cartões encontrados com sucesso",
    				content = @Content(mediaType = "application/json", array = @ArraySchema(schema = @Schema(implementation = CartaoModel.class))))
    @GetMapping("/")
    ...
    @ApiResponse(responseCode = "201", description = "Cartão salvo com sucesso",
            content = @Content(mediaType = "application/json", schema = @Schema(implementation = CartaoModel.class)))
    @PostMapping("/")
    ...
    @PutMapping("/{id}")
    public ResponseEntity<Object> put(@PathVariable Long id, @Valid @RequestBody CartaoDto cartaoDto, BindingResult result){
    	Optional<CartaoModel> optionalCartao = cartaoService.findById(id);
