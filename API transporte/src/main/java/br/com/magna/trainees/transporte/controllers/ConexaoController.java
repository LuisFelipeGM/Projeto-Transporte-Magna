...
public class ConexaoController extends GenericController {
	...
	@ApiResponse(responseCode = "200", description = "Conexões encontradas com sucesso", content = @Content(mediaType = "application/json", array = @ArraySchema(schema = @Schema(implementation = ConexaoModel.class))))
	@GetMapping("/")
	...
	@ApiResponse(responseCode = "200", description = "Conexão encontrada com sucesso", content = @Content(mediaType = "application/json", array = @ArraySchema(schema = @Schema(implementation = ConexaoModel.class))))
	...
	@ApiResponse(responseCode = "201", description = "Conexão salva com sucesso", content = @Content(mediaType = "application/json", schema = @Schema(implementation = CartaoModel.class)))
	@PostMapping("/")
		...
		try {
			return result.hasErrors() ? ResponseEntity.unprocessableEntity().body(getErrors(result))
					: ResponseEntity.status(HttpStatus.CREATED)
							.body(conexaoService.adicionaConexaoEstacaoLinha(conexaoDto));
		} catch (Exception e) {
	...
	public ResponseEntity<Object> delete(@PathVariable Long id) {
		...
		return optionalConexao.map(conexao -> {
